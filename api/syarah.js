export default async function handler(req, res) {
    ...
    try {
        ...
        // Cek jika Google mengirim error (seperti Limit 429)
        if (data.error) {
            if (data.error.code === 429) {
                return res.status(429).json({ error: 'Limit API Gemini habis, tunggu sebentar ya.' });
            }
        ...
        res.status(500).json({ error: 'Koneksi ke server AI terputus.' });
    }
}
